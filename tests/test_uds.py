...
class TestKeaSocketClientReload(unittest.TestCase):
    ...
    @patch('sys.exit')
    @patch('click.echo')
    def test_reload_unsupported_config(self, mock_echo, mock_exit, mock_abspath, mock_access, mock_socket_class):
        ...
        # Should have echoed error message and exited
...
class TestKeaSocketClientStats(unittest.TestCase):
    ...
    def test_stats_dhcp6(self, mock_abspath, mock_access, mock_socket_class):
        ...
        config_data = {
            ...
            "arguments": {
                "Dhcp6": {
                    "subnet6": [{"id": 10, "subnet": "2001:db8::/64"}]
                }
            }
        }
    ...
    def test_stats_calls_reload(self, mock_abspath, mock_access, mock_socket_class):
        ...
        mock_file = MagicMock()
        mock_file.read.side_effect = [
            json.dumps(config_data),  # First reload
            json.dumps(stats_data)     # Stats query
        ]
        mock_sock.makefile.return_value = mock_file
