...
- **Frontier query**: list the map's open children (`gh issue list --state open --json number,title,assignees,blockedBy`, scoped to the map's sub-issues / task list), drop any with a non-empty `blockedBy` or an assignee; first in map order wins. Reading the same fact over REST instead, the field is `.issue_dependencies_summary.blocked_by` (open blockers only). Where dependencies aren't available, fall back to an open issue named in the `Blocked by` line.
